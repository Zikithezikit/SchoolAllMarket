...
{   
    ...
    [WebMethod]
    public DataSet chkInfo(Credit chk)
    {
       
        OleDbConnection conn = new OleDbConnection(connString);
        string sqlstr = "SELECT tblCredit.fName, tblCredit.lName, tblCredit.adress, tblCredit.city FROM tblCredit WHERE(((tblCredit.id) ='" + chk.Id + "'));        ";
        OleDbDataAdapter da = new OleDbDataAdapter(sqlstr, conn);
        DataSet ds = new DataSet();
        da.Fill(ds);
        return ds;
